...
class Project {
    ...
    static checkCompilerWarnings(contracts, scripts, globalWarnings, changedContracts, forceRecompile, errorOnWarnings) {
    ...
    static needToUpdate(forceRecompile, changedContracts, name) {
    ...
    async saveArtifactsToFile(projectRootDir, forceRecompile, changedContracts) {
    ...
    async saveProjectArtifact(projectRootDir, forceRecompile, changedContracts) {
        if (!forceRecompile) {
    ...
    static async getDeployedContracts(mainnetNodeUrl, sourceInfos, artifactsRootDir) {
        const nodeProvider = new web3_1.NodeProvider(mainnetNodeUrl ?? defaultMainnetNodeUrl, undefined, fetchRetry.default(fetch, {
            retryOn: [429],
            retries: 3,
            retryDelay: 1000
        }));
        ...
        for (const sourceInfo of sourceInfos) {
            ...
            if (sourceInfo.type === SourceKind.Contract) {
                try {
                    ...
                    const contractCode = await (0, web3_1.getContractCodeByCodeHash)(nodeProvider, codeHash);
                    if (contractCode !== undefined)
                        result.push(artifact.name);
                }
                catch (error) {
    ...
    static async filterChangedContracts(mainnetNodeUrl, sourceInfos, artifactsRootDir, changedContracts, skipRecompileIfDeployedOnMainnet, skipRecompileContracts) {
        let deployedContracts;
        if (skipRecompileIfDeployedOnMainnet) {
            const remainSourceInfo = sourceInfos.filter((s) => s.type === SourceKind.Contract && !skipRecompileContracts.includes(s.name));
            deployedContracts = await this.getDeployedContracts(mainnetNodeUrl, remainSourceInfo, artifactsRootDir);
        }
        else {
            deployedContracts = [];
        }
        const filteredChangedContracts = [];
        changedContracts.forEach((c) => {
            if (skipRecompileContracts.includes(c)) {
                console.warn(`The contract ${c} is in the skipRecompileContracts list. Even if the contract is updated, the code will not be regenerated. ` +
                ...
                console.warn(`The contract ${c} has already been deployed to the mainnet. Even if the contract is updated, the bytecode will not be regenerated. ` +
    ...
    static async compile_(fullNodeVersion, provider, sourceInfos, projectRootDir, contractsRootDir, artifactsRootDir, errorOnWarnings, compilerOptions, changedContracts, forceRecompile, skipRecompileIfDeployedOnMainnet, skipRecompileContracts, mainnetNodeUrl) {
        ...
        Project.checkCompilerWarnings(contracts, scripts, result.warnings ?? [], changedContracts, forceRecompile, errorOnWarnings);
        ...
        const filteredChangedContracts = await Project.filterChangedContracts(mainnetNodeUrl, sourceInfos, artifactsRootDir, changedContracts, skipRecompileIfDeployedOnMainnet, skipRecompileContracts);
        await project.saveArtifactsToFile(projectRootDir, forceRecompile, filteredChangedContracts);
        return project;
    }
    static async loadArtifacts(provider, sourceInfos, projectRootDir, contractsRootDir, artifactsRootDir, errorOnWarnings, compilerOptions, changedContracts, forceRecompile, skipRecompileIfDeployedOnMainnet, skipRecompileContracts, mainnetNodeUrl) {
        ...
        try {
            ...
            return Project.compile_(projectArtifact.fullNodeVersion, provider, sourceInfos, projectRootDir, contractsRootDir, artifactsRootDir, errorOnWarnings, compilerOptions, changedContracts, forceRecompile, skipRecompileIfDeployedOnMainnet, skipRecompileContracts, mainnetNodeUrl);
        }
    }
    ...
    static async compile(compilerOptionsPartial = {}, projectRootDir = '.', contractsRootDir = Project.DEFAULT_CONTRACTS_DIR, artifactsRootDir = Project.DEFAULT_ARTIFACTS_DIR, defaultFullNodeVersion = undefined, forceRecompile = false, skipRecompileIfDeployedOnMainnet = false, skipRecompileContracts = [], mainnetNodeUrl = undefined) {
        ...
        if (forceRecompile ||
            ...
            await Project.compile_(fullNodeVersion, provider, sourceFiles, projectRootDir, contractsRootDir, artifactsRootDir, errorOnWarnings, nodeCompilerOptions, changedContracts, forceRecompile, skipRecompileIfDeployedOnMainnet, skipRecompileContracts, mainnetNodeUrl);
        }
        // we need to reload those contracts that did not regenerate bytecode
        return await Project.loadArtifacts(provider, sourceFiles, projectRootDir, contractsRootDir, artifactsRootDir, errorOnWarnings, nodeCompilerOptions, changedContracts, forceRecompile, skipRecompileIfDeployedOnMainnet, skipRecompileContracts, mainnetNodeUrl);
    }
}
