...
async function deploy(configuration, networkId, deployments, fromIndex, toIndex, silent = false) {
    ...
    if (configuration.skipRecompileOnDeployment !== true) {
        project = await project_1.Project.compile(configuration.compilerOptions, path_1.default.resolve(process.cwd()), configuration.sourceDir ?? types_1.DEFAULT_CONFIGURATION_VALUES.sourceDir, artifactDir, undefined, configuration.forceRecompile, configuration.skipRecompileIfDeployedOnMainnet, configuration.skipRecompileContracts ?? [], configuration.networks['mainnet'].nodeUrl);
    }
